...
from app.celery.tasks import send_password_reset_otp_email
...
from app.utils.auth_helpers import change_user_password, create_user_dict, get_password_hash, send_and_save_otp, verify_otp_attempts, verify_password, create_access_token, create_refresh_token, decode_token, generate_otp
...
async def register(user: UserCreate, db: AsyncIOMotorDatabase = Depends(get_db)):
    ...
    user_dict = create_user_dict(user, hashed_password)

    await db["users"].insert_one(user_dict)
    await send_and_save_otp(user.email, db)
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncIOMotorDatabase = Depends(get_db)):
    ...
    refresh_token = create_refresh_token(data={"sub": user["email"]}, token_version=user['token_version'])
...
async def refresh_access_token(refresh_token_request: RefreshTokenRequest, db: AsyncIOMotorDatabase = Depends(get_db)):
    ...
    new_access_token = create_access_token(data={"sub": email}, token_version=token_version)
...
async def resend_otp(
    ...
    await send_and_save_otp(current_user.email, db)
